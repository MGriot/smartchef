...
vi.mock('../gitfs', () => ({ getSyncBasePath: async () => '/chosen-folder/SmartChef' }));
...
describe('createElectronRemoteTransport', () => {
  it('resolves paths against the chosen Sync Folder (getSyncBasePath())', async () => {
    ...
    expect(getLocalText(remoteFs, '/chosen-folder/SmartChef/.git/objects/ab/cdef01')).toBe('object-bytes');
  });
  ...
  it('exists() is true for a present file, false for a missing one', async () => {
    putLocalText(remoteFs, '/chosen-folder/SmartChef/.git/refs/heads/main', 'commit-abc\n');
    const transport = await createElectronRemoteTransport();
  ...
  it('listDir returns entry names, [] for a missing directory', async () => {
    putLocalText(remoteFs, '/chosen-folder/SmartChef/.git/objects/ab/cdef01', 'x');
    const transport = await createElectronRemoteTransport();
