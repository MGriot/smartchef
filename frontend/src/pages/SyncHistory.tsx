    ...
    <AppLayout>
      <div className="p-6 sm:p-12 max-w-3xl mx-auto">
        <div className="mb-10">
          ...
          <h1 className="text-4xl font-black text-zinc-900 tracking-tighter">Sync History</h1>
        ...
        {devices.length > 0 && (
          <div className="bg-white rounded-[40px] p-6 sm:p-10 shadow-sm border border-zinc-100 mb-8">
            <h2 className="text-lg font-black text-zinc-900 mb-4">Devices</h2>
              ...
              {devices.map((d) => (
                <div key={d.deviceId} className="flex items-center justify-between px-4 py-2.5 bg-zinc-50 rounded-xl">
                  <span className="flex items-center gap-2 text-sm font-bold text-zinc-700">
                    <span className="material-symbols-outlined text-[18px] text-zinc-400">{PLATFORM_ICON[d.platform]}</span>
                    {d.deviceName}
                  </span>
                  <span className="text-xs text-zinc-400">{formatWhen(new Date(d.lastSyncAt).getTime())}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {error && (
          <div className="bg-white rounded-[40px] p-10 shadow-sm border border-zinc-100 text-center text-sm text-red-600 font-medium">
            {error}
          </div>
        )}
        ...
        {!error && commits !== null && commits.length === 0 && (
          <div className="bg-white rounded-[40px] p-10 shadow-sm border border-zinc-100 text-center">
            <p className="text-sm text-zinc-400 font-medium">No sync activity yet — this shows up after the first save or "Sync Now".</p>
          </div>
        )}
        ...
        {!error && commits !== null && commits.length > 0 && (
          <div className="space-y-3">
            {commits.map((c) => (
              <div key={c.oid} className="bg-white rounded-3xl border border-zinc-100 shadow-sm overflow-hidden">
                <button
                  type="button"
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-bold text-zinc-900 truncate">{c.message}</p>
                    <p className="text-xs text-zinc-400 font-medium mt-0.5">{c.authorName} · {formatWhen(c.timestamp)}</p>
                  </div>
                  <span className="material-symbols-outlined text-zinc-400 shrink-0">
                ...
                {expanded === c.oid && (
                  <div className="px-5 pb-5 space-y-1.5">
                    {c.changedFiles.length === 0 && (
                      <p className="text-xs text-zinc-400">No file changes recorded for this commit.</p>
                    )}
                    {c.changedFiles.map((f) => (
                      <div key={f} className="px-3 py-2 bg-zinc-50 rounded-lg text-xs font-medium text-zinc-600">
                        {describeChange(f)}
                      </div>
                    ))}
                  </div>
