    ...
    <AppLayout>
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-headline font-extrabold text-zinc-900 capitalize">{monthLabel}</h1>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setMonth((m) => new Date(m.getFullYear(), m.getMonth() - 1, 1))}
              className="w-10 h-10 rounded-full bg-white shadow-sm border border-zinc-100 flex items-center justify-center text-zinc-500 hover:text-primary transition-colors"
            >
              <span className="material-symbols-outlined">chevron_left</span>
            </button>
            <button
              onClick={() => setMonth(() => { const now = new Date(); return new Date(now.getFullYear(), now.getMonth(), 1); })}
              className="px-4 py-2 rounded-full bg-white shadow-sm border border-zinc-100 text-xs font-bold text-zinc-500 hover:text-primary transition-colors"
            >
              {t('history.today')}
            </button>
            <button
        ...
        <div className="grid grid-cols-7 gap-2 mb-2">
          {weekdayLabels.map((label) => (
            <div key={label} className="text-center text-[10px] font-bold uppercase tracking-widest text-zinc-400 py-1">
              {label}
            </div>
          ))}
        </div>
          ...
          {gridDays.map((day) => {
            ...
            return (
              <button
                ...
                className={`aspect-square rounded-2xl p-2 flex flex-col items-start bg-white border transition-all text-left ${
              ...
              >
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {dayEntries.length > MAX_THUMBS_PER_DAY && (
                    <span className="w-6 h-6 rounded-lg bg-zinc-100 text-zinc-500 text-[9px] font-bold flex items-center justify-center">
                      +{dayEntries.length - MAX_THUMBS_PER_DAY}
                    </span>
                  )}
                </div>
        ...
        {loading && <p className="text-center text-zinc-400 text-sm mt-8">{t('common.loading')}</p>}
        {!loading && entries.length === 0 && (
          <p className="text-center text-zinc-400 text-sm mt-8">{t('history.noEntriesThisMonth')}</p>
        )}
      </div>
      ...
      {selectedDay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" onClick={() => setSelectedDay(null)}>
          ...
          <div
            className="relative bg-white rounded-3xl shadow-2xl p-6 max-w-md w-full max-h-[70vh] overflow-y-auto"
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSelectedDay(null)} className="text-zinc-400 hover:text-zinc-700">
              ...
              {selectedEntries.map((e) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {e.cookedByName && <p className="text-xs text-zinc-400">{t('history.cookedBy', { name: e.cookedByName })}</p>}
                  </div>
                </Link>
              ))}
            </div>
