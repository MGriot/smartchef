    ...
    <div className="min-h-screen bg-[#fafaf5] flex items-center justify-center p-6 font-outfit">
      <div className="w-full max-w-md bg-white rounded-[40px] shadow-sm border border-zinc-100 p-10">
        <div className="mb-8 text-center">
          ...
          <p className="text-sm text-zinc-400 font-medium">{creating ? 'Create a new profile' : "Who's cooking?"}</p>
        </div>
        ...
        {!creating && (
          <>
            {profiles === null ? (
              <p className="text-center text-sm text-zinc-400 py-6">Loading profiles…</p>
            ) : profiles.length === 0 ? (
              <p className="text-center text-sm text-zinc-400 py-6">No profiles yet on this library.</p>
            ) : (
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {AVATAR_PRESETS.map((preset) => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-full overflow-hidden shrink-0 transition-all ${newAvatar === preset ? 'ring-4 ring-primary' : 'ring-2 ring-transparent hover:ring-zinc-200'}`}
                    >
