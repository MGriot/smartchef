...
export default function LibraryTools() {
  ...
  const [form, setForm] = useState({ name: '', category: '', description: '', icon: 'FaKitchenSet' });
  ...
  const handleOpenModal = (tool: any = null) => {
    if (tool) {
      ...
      setEditingTool(null);
      setForm({ name: '', category: '', description: '', icon: 'FaKitchenSet' });
    }
    setShowModal(true);
  };
                    ...
                    <tr key={tool.id} className="group hover:bg-zinc-50/50 transition-colors">
                      <td className="py-6 pl-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-extrabold text-zinc-900 leading-tight">{tool.name}</p>
